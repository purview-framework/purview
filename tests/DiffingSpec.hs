  ...
  describe "diff" $ do
    ...
    describe "message handlers" $ do
      it "doesn't diff handler children if the state is the same" $ do
        let
          mkHandler :: (String -> Purview parentAction action IO) -> Purview parentAction action IO
      ...
      it "diffs handler children if the state is different" $ do
        let
          handler1 :: (String -> Purview parentAction action IO) -> Purview parentAction action IO
