...
spec = parallel $ do
  describe "render" $ do
    it "can create a div" $ do
      let element = div [Text "hello world"]
    ...
    it "can add an onclick" $ do
      ...
      render element
        `shouldBe` "<div action=1>hello world</div>"
    ...
    it "can render a message handler" $ do
      let actionHandler :: String -> Int -> Int
    ...
    it "can render composed styles" $ do
      ...
      render (blue . halfSize $ div [text "box"])
        `shouldBe` "<div style=\"width: 50%; height: 50%;color: blue;\">box</div>"
  ...
  describe "applyEvent" $ do
    it "changes state" $ do
      ...
      render appliedHandler
        `shouldBe` "<div handler=\"null\">1</div>"

    it "works with typed messages" $ do
      let actionHandler :: TestAction -> Int -> Int
          actionHandler Up _ = 1
  ...
  describe "prepareGraph" $ do
    it "sets hasRun to True" $ do
      ...
      show (fst (prepareGraph component))
        `shouldBe` "EffectHandler Just [] Once True div [  \"Nothing\" Attr div [  \"check time\" ]  ] "
    ...
    it "stops collecting the action if it has already run" $ do
      ...
      let run1 = prepareGraph component
    ...
    it "assigns a location to handlers" $ do
      let timeHandler = effectHandler Nothing handle
      ...
      let graphWithLocation = fst (prepareGraph component)
